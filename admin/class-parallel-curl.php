...
//
// To use it, first create the ParallelCurlClass object:
//
// $parallelCurl = new SelfWorks\ParallelCURL\ParallelCurlClass(10);
//
...
class ParallelCurl {
    ...
    private function checkForCompletedRequests()
    {
        ...
        while ($info = curl_multi_info_read($this->multi_handle)) {
            ...
            // $call_calback_instance = new $callback['class'];
            // $call_calback_instance->{$callback['function']}($content, $url, $ch, $user_data);

            $this->requested_contents[$callback['request_id']] = $content;
